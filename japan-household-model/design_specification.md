...
3. **総供給量 ($Y_{s, t}$)**
   $$Y_{s, t} = Y_{\text{potential}, t} \times \max\left(0.5, 1 - \beta_1 (W_{\text{min}, t} - 1.0) - \beta_2 (R_{\text{long}, t-1} - R_{\text{neutral}})\right)$$
   - $\beta_1 = 0.1$（最低賃金による供給弾性値。最賃が10%上がると供給が1%収縮）
...
4. **生活実感インフレ率 ($\pi_{\text{living}, t}$)**
   ...
   $$\pi_{\text{living}, t} = \pi_{\text{clamped}} + 0.12 \times \max\left(0, \frac{E_t - E_{\text{init}}}{E_{\text{init}}}\right) \times (1 - Self\_Suff)$$
   - $Self\_Suff$: 食料自給率スライダー (初期値 0.38)

### 2.8 労働市場・就業調整・リスキリング動学
最低賃金上昇に伴う就業調整（年収 of 壁）および、政府支援による労働移動の動学です。
