...
2. **インフレ率 ($\pi_t$)**
   ...
   - $\kappa = 0.20$（需給ギャップインフレ感応度。供給不足にともなうディマンド・プル圧力の表現を強めるため、過度な振幅を抑え0.20へマイルド化）
   - $\lambda = 0.24$（限界費用インフレ感応度。コストプッシュインフレを表現。価格転嫁率 $\alpha_{\text{pass}}$ に比例。過敏な急騰を防ぐため0.24へ平滑化）
...
1. **総需要量 ($Y_{\text{demand}, t}$)**
   ...
   - $\theta = 0.6$（金利の需要抑制効果。金利1%上昇で総需要が0.6%減少する。急激な需要冷え込みを防ぐため0.6へ平滑化）
