...
3. **総供給量 ($Y_{s, t}$)**
   ...
   - $R_{\text{neutral}} = 0.026$（中立長期金利 2.6%）
   - $\max(0.5, \dots)$: 供給の崩壊を防ぐ下限ガード

### 2.2 コストプッシュ型インフレ動学
インフレ率 $\pi_t$ は、マクロの需給ギャップ、企業の限界費用の上昇による価格転嫁、為替変動、および消費税調整に伴う直接的な物価押し上げ効果（一時的ショック）から動的に決定されます。
...
2. **インフレ率 ($\pi_t$)**
   ...
   - $\text{tax\_shock}_t$: 消費税率変更に伴う一時的（減衰型）の直接価格上乗せ効果。消費税の増減税（$dConsumption$）は、導入時（第0期・第1期）にインフレ率を直接押し上げますが、その後は四半期ごとに $0.5$ の比率で急激に減衰（$\times 0.5^t$）し、直接的なインフレ押し上げ効果は消失します。
...
2. **長期金利 ($R_{\text{long}, t}$)**
   ...
   - $term\_premium = 0.016$（期間プレミアム 1.6%）
...
2. **価格デフレーター ($P_t$)**
   ...
   - $P_0 = 1.0$ （初期値。消費税による初期の強制ジャンプは行いません）

3. **実質可処分所得 ($YD_{\text{real}, t}$)**
   $$YD_{\text{real}, t} = \frac{YD_t}{P_t}$$
   - インフレ率（消費税の一時的ショックを含む）が毎期蓄積することで物価デフレーターが上昇し、名目可処分所得 $YD_t$ が実質化（手取り購買力）されます。
...
3. **初期インフレ率 ($\pi_{0}$)**
   ...
   $$\pi_{0, clamped} = \max\left(-\frac{0.05}{4}, \min\left(\frac{0.15}{4}, \pi_0\right)\right)$$

4. **初期政策金利 ($r_{\text{policy}, 0}$)**
   $$r_{\text{policy}, 0} = \max\left(0.0, \min\left(0.12, r_{\text{neutral}} + 1.2 \times \left(\pi_{0, clamped} - \frac{\pi_{\text{target}}}{4}\right) + 0.3 \times init\_gap\_ratio\right)\right)$$
