...
### Status
- **Phase 6: realigned** - 完了 =^・^= ⭐
  - 金利感応度の調整（0.3 -> 0.1）および四半期ラグの導入により、インフレ金利の4期ごと鋸歯状（ギザギザ）振動をマイルド化。
  - チャート2（家計負担・構成）を左右2軸化し、左Y軸（実質可処分所得用）、右Y軸（ローン支払負担・預金利息受取用：0〜10兆円）に分離して変化を視覚化。
